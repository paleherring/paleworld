...
class FLAGS(object):
    def __init__(self):
        ...
        self.z_dim = 100 # Number of noise value
        ...
        self.save_every_epoch = 1 # The interval of saving checkpoints.
        self.dataset = "celebA" #The name of the dataset [celebA,mnist,lsun]
        self.checkpoint_dir = "checkpoint" # Directory name to save the checkpoints
        self.sample_dir = "samples" #Directory name to save the image samples
        assert np.sqrt(self.sample_size) % 1 == 0.,'Flag `sample_size` needs to be a perfect square'

flags = FLAGS()

tl.files.exists_or_mkdir(flags.checkpoint_dir) # save the model
tl.files.exists_or_mkdir(flags.sample_dir) # save generated image

def get_celebA(output_size, n_epoch, batch_size):
    # dataset API and augmentation
    images_path = tl.files.load_file_list(path='/Users/apple/Desktop/CS_Bella/Replicating_Work/dcgan-master/data', regx='.*.jpg', keep_prefix=True, printable = False)
    def generator_train():
